...
Проект включает готовое Docker окружение с предустановленными инструментами:
...
# 3. Запуск инфраструктуры (PostgreSQL, Grafana, Prometheus)
docker compose up -d
...
| Команда | Описание |
|---------|----------|
| `npm run sdk:build` | Сборка SDK |
| `npm run sdk:build:hard` | Полная пересборка SDK (с очисткой структур) |
| `npm run sdk:push` | Публикация SDK в npm (минорная версия) |
| `npm run sdk:push:major` | Публикация SDK в npm (мажорная версия) |
...
Для валидации входных данных в контроллерах используются декораторы из `@nestia/core` вместо стандартных. Внутри этот модуль использует Typia, поэтому поддерживается вся её валидация.
...
- **Генерация файла:** `npm run build:swagger`
...
| Команда | Описание |
|---------|----------|
| `npm run prisma:format` | Форматирование схемы Prisma |
| `npm run prisma:generate` | Генерация Prisma Client |
| `npm run prisma:create` | Создание новой миграции (без применения) |
| `npm run prisma:dev` | Создание и применение миграции в dev режиме |
| `npm run prisma:deploy` | Применение миграций в продакшн |
| `npm run prisma:down` | Генерация SQL для отката миграций |
| `npm run prisma:seed` | Заполнение базы данных тестовыми данными |
...
| Команда | Описание |
|---------|----------|
| `npm run update` | Проверка доступных обновлений (без установки) |
| `npm run sdk:update` | Проверка обновлений для SDK пакета |
| `npm run sdk:update:run` | Автоматическое обновление SDK зависимостей |
